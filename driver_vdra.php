...
// 1. Prepare SQL to fetch the VehicleID
$sqlVehicleID = "
SELECT VehicleID 
...
// Prepare the statement for fetching VehicleID
$stmtVehicleID = oci_parse($conn, $sqlVehicleID);

// Bind the variable for username
oci_bind_by_name($stmtVehicleID, ':username', $username);

// Execute the statement
$r = oci_execute($stmtVehicleID);

if (!$r) {
    $m = oci_error($stmtVehicleID);
    echo "<p>Failed to execute query to fetch VehicleID: " . htmlentities($m['message'], ENT_QUOTES) . "</p>";
    exit; // Exit or handle the error as needed
}

// Fetch the VehicleID
$vehicleID = null;
if ($row = oci_fetch_array($stmtVehicleID, OCI_ASSOC)) {
    $vehicleID = $row['VEHICLEID']; // Make sure the column name matches the case
} else {
...
// Free the statement
oci_free_statement($stmtVehicleID);

// Handle form submission for adding VDRA data
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['VDRA_Date'])) {
    // Get form data
    ...
    $sql = "BEGIN update_vdra_km(:vehicleID, TO_DATE(:VDRA_Date, 'YYYY-MM-DD'), :Route, :KM_Reading); END;";
            ...
            <div id="content">
                ...
                <div class="container-fluid" id="container-wrapper">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">VDRA</h1>
                        <ol class="breadcrumb">
        ...
        <div class="modal-content">
            <form method="POST">
                <div class="modal-header">
                    <h5 class="modal-title" id="addVDRAModalLabel">Add Your Vehicle VDRA Entry</h5>
