...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateDriverID'])) {
    $DriverID = $_POST['updateDriverID'];
    $RoleID = isset($_POST['updateFirst_Name']) ? $_POST['updateFirst_Name'] : '';
    $Name = isset($_POST['updateLast_Name']) ? $_POST['updateLast_Name'] : '';
    $Rank = isset($_POST['updateRank']) ? $_POST['updateRank'] : '';
    $Unit = isset($_POST['updateUnit']) ? $_POST['updateUnit'] : '';
    $LicenseNumber = isset($_POST['updateLicenseNumber']) ? $_POST['updateLicenseNumber'] : '';
    $ContactInfo = isset($_POST['updateContactInfo']) ? $_POST['updateContactInfo'] : '';

    $sql = "UPDATE driver SET First_Name = :First_Name, Last_Name = :Last_Name, Rank = :Rank, Unit = :Unit, 
            LicenseNumber = :LicenseNumber, ContactInfo = :ContactInfo WHERE DriverID = :DriverID";
    $stid = oci_parse($conn, $sql);
    ...
    if (!$r) {
        ...
        echo "<p>Driver updated successfully!</p>";
    }

    oci_free_statement($stid);
}

// Handle form submission for deleting a driver
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['deleteDriverID'])) {
    $DriverID = $_POST['deleteDriverID'];
    ...
    if (!$r) {
        ...
        echo "<p>Driver deleted successfully!</p>";
    }
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid" id="container-wrapper">
                    ...
                    <!-- Update Driver Modal -->
                    <div class="modal fade" id="updatedriverModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                         aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel" style="color: #4B5320;">Update Driver</h5>
                                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">×</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <form action="" method="POST">
                                        ...
                                        <div class="form-group">
                                            <label for="updateRoleID" class="col-form-label">First Name:</label>
                                            <input type="text" class="form-control" id="updateFirst_Name" name="updateFirst Name" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="updateName" class="col-form-label">Last Name:</label>
                                            <input type="text" class="form-control" id="updateLast_Name" name="updateLast_Name" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="updateRank" class="col-form-label">Rank:</label>
                                            <input type="text" class="form-control" id="updateRank" name="updateRank" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="updateLicenseNumber" class="col-form-label">License Number:</label>
                                            <input type="text" class="form-control" id="updateLicenseNumber" name="updateLicenseNumber" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="updateContactInfo" class="col-form-label">Contact Info:</label>
                                            <input type="text" class="form-control" id="updateContactInfo" name="updateContactInfo" required>
                                        </div>
                                        <div class="modal-footer">
                                            <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
                    ...
                    <!-- Delete Driver Modal -->
                    <div class="modal fade" id="deletedriverModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                         aria-hidden="true">
                             <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                     <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel" style="color: #4B5320;">Delete Driver</h5>
                                           <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                                               <span aria-hidden="true">×</span>
                                          </button>
                                    </div>
                                <div class="modal-body">
                                     <form action="" method="POST">
                                       <input type="hidden" id="deleteDriverID" name="deleteDriverID">
                                          <p id="deleteMessage">Are you sure you want to delete this driver?</p>
                                 <div class="modal-footer">
                                      <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
    ...
    <script>
        ...
        function populateUpdateModal(data) {
            $('#updateDriverID').val(data['DRIVERID']);
            $('#updateFirst_Name').val(data['FIRST_NAME']);
            $('#updateLast_Name').val(data['LAST_NAME']);
            $('#updateRank').val(data['RANK']);
            $('#updateUnit').val(data['UNIT']);
            $('#updateLicenseNumber').val(data['LICENSENUMBER']);
            $('#updateContactInfo').val(data['CONTACTINFO']);
            $('#updatedriverModal').modal('show');
        }

        function populateDeleteModal(data) {
    $('#deleteDriverID').val(data['DRIVERID']);
    var deleteMessage = "Are you sure you want to delete " + data['RANK'] + " " + data['NAME'] + "?";
