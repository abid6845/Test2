...
// Handle form submission for updating a POL issue
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateIssueID'])) {
    $IssueID = $_POST['updateIssueID'];
    $IssueDate = isset($_POST['updateIssueDate']) ? $_POST['updateIssueDate'] : '';
    $VehicleID = isset($_POST['updateVehicleID']) ? $_POST['updateVehicleID'] : '';
    $POL_Grade = isset($_POST['updatePOL_Grade']) ? $_POST['updatePOL_Grade'] : '';
    $IssueAmount = isset($_POST['updateIssueAmount']) ? $_POST['updateIssueAmount'] : '';
    $POL_ID = isset($_POST['updatePOL_ID']) ? $_POST['updatePOL_ID'] : '';

    $sql = "UPDATE POL_Issue 
            SET IssueDate = TO_DATE(:IssueDate, 'YYYY-MM-DD'), VehicleID = :VehicleID, POL_Grade = :POL_Grade, IssueAmount = :IssueAmount, POL_ID = :POL_ID 
            WHERE IssueID = :IssueID";
    $stid = oci_parse($conn, $sql);

    oci_bind_by_name($stid, ':IssueID', $IssueID);
    oci_bind_by_name($stid, ':IssueDate', $IssueDate);
    oci_bind_by_name($stid, ':VehicleID', $VehicleID);
    oci_bind_by_name($stid, ':POL_Grade', $POL_Grade);
    oci_bind_by_name($stid, ':IssueAmount', $IssueAmount);
    ...
    if (!$r) {
        ...
        echo "<p>POL Issue updated successfully!</p>";
    }

    oci_free_statement($stid);
}

// Handle form submission for deleting a POL issue
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['deleteIssueID'])) {
    $IssueID = $_POST['deleteIssueID'];

    $sql = "DELETE FROM POL_Issue WHERE IssueID = :IssueID";
    $stid = oci_parse($conn, $sql);
    ...
    if (!$r) {
        ...
        echo "<p>POL Issue deleted successfully!</p>";
    }
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid" id="container-wrapper">
                                ...
                                <div class="table-responsive p-3">
                                    <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                                        ...
                                        while ($row = oci_fetch_assoc($stid)) {
                                            ...
                                            echo '<td>
                                                    <a href="#" class="btn btn-success btn-sm" onclick="populateUpdateModal(' . htmlspecialchars(json_encode($row), ENT_QUOTES) . ')">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="#" class="btn btn-danger btn-sm ms-2" onclick="populateDeleteModal(' . htmlspecialchars(json_encode($row), ENT_QUOTES) . ')">
                    ...
                    <!-- Update POL Issue Modal -->
                    <div class="modal fade" id="updatePOLIssueModal" tabindex="-1" role="dialog" aria-labelledby="updatePOLIssueModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="updatePOLIssueModalLabel">Update POL Issue</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <form method="POST" action="pol_issue.php"> <!-- Update with your PHP script -->
                                    <div class="modal-body">
                                        <input type="hidden" name="updateIssueID" id="updateIssueID">
                                        <div class="form-group">
                                            <label for="updateIssueDate">Issue Date:</label>
                                            <input type="date" class="form-control" name="updateIssueDate" id="updateIssueDate" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="updateVehicleID">Vehicle ID:</label>
                                            <input type="text" class="form-control" name="updateVehicleID" id="updateVehicleID" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="updatePOL_Grade">POL Grade:</label>
                                            <input type="text" class="form-control" name="updatePOL_Grade" id="updatePOL_Grade" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="updateIssueAmount">Issue Amount:</label>
                                            <input type="number" class="form-control" name="updateIssueAmount" id="updateIssueAmount" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="updatePOL_ID">POL ID:</label>
                                            <input type="text" class="form-control" name="updatePOL_ID" id="updatePOL_ID" required>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                        <button type="submit" class="btn btn-primary">Update POL Issue</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    <!-- Delete POL Issue Modal -->
                    <div class="modal fade" id="deletePOLIssueModal" tabindex="-1" role="dialog" aria-labelledby="deletePOLIssueModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="deletePOLIssueModalLabel">Delete POL Issue</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <form method="POST">
                                    <div class="modal-body">
                                        <input type="hidden" name="deleteIssueID" id="deleteIssueID">
                                        <p>Are you sure you want to delete this POL issue?</p>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                        <button type="submit" class="btn btn-danger">Delete POL Issue</button>
                                    </div>
                                </form>
                            </div>
                        </div>
    ...
    <script>
        function populateUpdateModal(data) {
            document.getElementById('updateIssueID').value = data.ISSUEID;
            document.getElementById('updateIssueDate').value = data.ISSUEDATE;
            document.getElementById('updateVehicleID').value = data.VEHICLEID;
            document.getElementById('updatePOL_Grade').value = data.POL_GRADE;
            document.getElementById('updateIssueAmount').value = data.ISSUEAMOUNT;
            document.getElementById('updatePOL_ID').value = data.POL_ID;

            $('#updatePOLIssueModal').modal('show');
        }

        function populateDeleteModal(data) {
            document.getElementById('deleteIssueID').value = data.ISSUEID;
            $('#deletePOLIssueModal').modal('show');
        }
    </script>
